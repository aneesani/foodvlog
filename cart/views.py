...
def add_cart(request,product_id):
    ...
    try:
        ...
        ct.save()
    try:
        c_items=cartitems.objects.get(prod=prodt,cart=ct)
        if c_items.quantity < c_items.prod.stock:
            c_items.quantity+=1
        c_items.save()

    except cartitems.DoesNotExist:
        c_items=cartitems.objects.create(prod=prodt,quantity=1,cart=ct)
        c_items.save()
...
def min_cart(request,product_id):
  ...
  c_items=cartitems.objects.get(prod=prod,cart=ct)
